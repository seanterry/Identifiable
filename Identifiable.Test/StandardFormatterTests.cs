...
partial class TimeGuidFormatterTests
{
    ...
    public class StandardFormatterTests : TimeGuidFormatterTests
    {
        protected override TimeGuid.IFormatter instance() => new TimeGuid.StandardFormatter();
        ...
        public void Returns_bytes_0_to_7_as_time_with_version_1( long time )
        {
            ...
            Assert.Equal( expected[..7], bytes[..7] );

            // version is set in the most significant nibble of byte 7
            Assert.Equal( ( expected[7] & 0x0f ) | 0x10, bytes[7] );
        }
        ...
        [InlineData( 0x1234 )]
        public void Returns_bytes_8_to_9_as_clock_with_variant_1( short clock )
        {
            this.clock = clock;
            var result = method();
            var bytes = result.ToByteArray();
            ...
            if ( !BitConverter.IsLittleEndian ) Array.Reverse( expected );
            Assert.Equal( ( expected[0] & 0xbf ) | 0x80, bytes[8] );
